...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{$D = 1$ (Demand: High)}: The league will draft almost every available player. One greedy mistake means replacing a starter with absolute zero value.
...
S13 & Demand: High & \textbf{1} & Normal & Roster-Ratio & (0, 30) \\
S14 & Demand: Low & \textbf{10} & Normal & Roster-Ratio & (0, 30) \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{Factor C: Market Conditions and ADP Uncertainty results. The first sweep tests market noise ($\sigma_{\text{adp}}$) with fixed $\delta=0$. The second tests tolerance ($\delta$) with fixed $\sigma_{\text{adp}}=30$. Values represent the optimality gap (\%) $\pm$ standard deviation.}
\label{tab:factor_3}
\begin{tabular}{llccc}
\toprule
...
\centering
\small
\caption{Stress Test: DG and OCG Optimality Gaps Relative to IP.}
...
\begin{figure}[H]
  ...
  \caption{Runtime comparison for Stress Testing. IP scales exponentially, while heuristics scale linearly.}
\end{figure}
...
\textbf{Market Conditions (Factor C)} & S10 & Market: Chaotic ($\delta=+5$) & \textbf{+4.16\%} \\
\textbf{Player Demand Ratio (Factor D)} & S13 & Demand: High ($D=1$) & \textbf{+6.82\%} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Pos: Single-Position (S6):} When positional flexibility is eliminated, early greedy mistakes are impossible to patch. OCG's foresight prevents these catastrophic positional deficits, maintaining high optimality.
    \item \textbf{Market: Chaotic (S10):} Even when opponents' behavior is unpredictable, OCG maintains a robust lead, proving its delay-cost heuristic acts as a natural hedge against uncertain market conditions.
    \item \textbf{Demand: High (S13):} In environments where almost every player is drafted ($D=1$), DG often suffers from "positional blindness," leading to a 6.82\% loss in roster value. OCG's foresight prevents these late-draft collapses.
\end{itemize}
...
While the exact ADP-aware IP provides flawless rosters, the limitless nature of the baseball player pool renders exact optimization operationally dead-on-arrival due to exponential runtime scaling. Our proposed \textbf{Opportunity Cost Greedy (OCG)} algorithm brilliantly bridges this gap. By calculating the delay-cost of waiting just one pick into the future, OCG captures the strategic essence of the IP. It consistently minimizes the optimality gap under severe market stress and processes 50-million variable equivalent draft instances in less than 25 seconds.
