...
\begin{table}[H]
\centering
\caption{Results for Factor A: Points Distribution. Gap values are optimal gap ratio ($\% \pm \text{std}$).}
\label{tab:factor_1}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[H]
\centering
\caption{Results for Factor B: Position Eligibility Distribution. Gap values are optimal gap ratio ($\% \pm \text{std}$).}
\label{tab:factor_2}
\begin{tabular}{lccc}
\toprule
Level & DG Gap (\%) & OCG Gap (\%) & Improvement (\%) \\
...
\begin{table}[H]
\centering
\caption{Factor D: Player Demand Ratio results (Fixed Roster Scale = 1). Gap values are optimal gap ratio ($\% \pm \text{std}$).}
\label{tab:factor_4}
\begin{tabular}{lccc}
\toprule
Level & DG Gap (\%) & OCG Gap (\%) & Improvement (\%) \\
...
\midrule
\textbf{Points Dist.} & S3 & Star-Heavy Market & \textbf{+6.10\%} \\
...
\begin{itemize}
    ...
    \item \textbf{Managing Rigid Rosters (S6):} When positional flexibility is eliminated, early greedy mistakes are impossible to patch. OCG's foresight prevents these catastrophic positional deficits, maintaining high optimality.
