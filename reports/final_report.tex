...
\toprule
\textbf{ID} & \textbf{Main Factor} & \textbf{Demand Ratio ($D$)} & \textbf{Points Dist. ($v_i$)} & \textbf{Pos. Dist. ($E_i$)} & \textbf{ADP Noise ($\delta, \sigma$)} \\ \midrule
...
\midrule
S9, 10 & Market: Chaotic & 3 & Normal & Roster-Ratio & \textbf{($\pm$5, 30)} \\
S11, 12 & Market: Chaotic & 3 & Normal & Roster-Ratio & \textbf{($\pm$10, 30)} \\ \midrule
S13 & Load (High) & \textbf{1} & Normal & Roster-Ratio & (0, 30) \\
S14 & Load (Low) & \textbf{3} & Normal & Roster-Ratio & (0, 30) \\
S15 & Load (Ultra-Low) & \textbf{10} & Normal & Roster-Ratio & (0, 30) \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Algorithms are evaluated using two primary metrics across 30 random seeds for each scenario:
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Optimal Gap Ratio (\%)}: The distance from the ILP benchmark, defined as $\frac{Z_{\text{ILP}} - Z_{\text{Heuristic}}}{Z_{\text{ILP}}} \times 100\%$. A lower value indicates higher accuracy.
...
\begin{table}[H]
\centering
\caption{Results for Factor A: Points Distribution. Values represent the optimality gap (\%) $\pm$ standard deviation.}
\label{tab:factor_1}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[H]
\centering
\caption{Results for Factor B: Position Eligibility Distribution. Values represent the optimality gap (\%) $\pm$ standard deviation.}
\label{tab:factor_2}
\begin{tabular}{lccc}
\toprule
Level & DG Gap (\%) & OCG Gap (\%) & Improvement (\%) \\
...
\begin{table}[H]
\centering
\caption{Factor C: ADP Uncertainty results. The first sweep tests market noise ($\sigma_{\text{adp}}$) with fixed $\delta=30$. The second tests tolerance ($\delta$) with fixed $\sigma_{\text{adp}}=30$. Values represent the optimality gap (\%) $\pm$ standard deviation.}
\label{tab:factor_3}
\begin{tabular}{llccc}
\toprule
Sweep & Level & DG Gap (\%) & OCG Gap (\%) & Improvement (\%) \\
\midrule
\textbf{Noise} & $\sigma_{\text{adp}}=0$ (Efficient) & $2.78\% \pm 1.33\%$ & $\mathbf{1.52\% \pm 1.15\%}$ & $1.00\% \pm 0.50\%$ \\
 & $\sigma_{\text{adp}}=30$ (Moderate) & $5.79\% \pm 1.35\%$ & $\mathbf{2.72\% \pm 1.39\%}$ & $3.02\% \pm 1.34\%$ \\
 & $\sigma_{\text{adp}}=60$ (Chaotic) & $5.10\% \pm 1.29\%$ & $\mathbf{2.92\% \pm 1.75\%}$ & $\mathbf{3.33\% \pm 1.43\%}$ \\
\midrule
\textbf{Tolerance} & $\delta=-10$ (Strict) & $4.65\% \pm 1.32\%$ & $\mathbf{1.90\% \pm 0.77\%}$ & $2.91\% \pm 1.71\%$ \\
 & $\delta=0$ (Neutral) & $4.79\% \pm 1.18\%$ & $\mathbf{1.92\% \pm 0.57\%}$ & $3.02\% \pm 1.34\%$ \\
 & $\delta=10$ (Permissive) & $5.42\% \pm 1.21\%$ & $\mathbf{1.72\% \pm 0.95\%}$ & $\mathbf{3.92\% \pm 1.03\%}$ \\
\bottomrule
\end{tabular}
\end{table}
...
D = 3 (Baseline Supply) & $4.79\% \pm 1.18\%$ & $\mathbf{1.92\% \pm 0.57\%}$ & $3.02\% \pm 1.34\%$ \\
...
timeout\_target & 184,320 & 17.760 & 23.286 & \textbf{1869.805} & \textbf{TIMEOUT} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
stress\_small  & 2.430\% & \textbf{1.098\%} \\
stress\_medium & 1.771\% & \textbf{0.646\%} \\
stress\_large  & 0.940\% & \textbf{0.407\%} \\
stress\_xlarge & 0.270\% & \textbf{0.137\%} \\
timeout\_target & -- & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\textbf{Points Dist. (Factor A)} & S3 & Star-Heavy Market & \textbf{+6.10\%} \\
\textbf{Positional (Factor B)} & S6 & Positional Rigidity & \textbf{+4.42\%} \\
\textbf{Market Noise (Factor C)} & S10 & Chaotic Market ($\delta=+5$) & \textbf{+4.16\%} \\
\textbf{Load/Scarcity (Factor D)} & S13 & Extreme Scarcity ($D=1$) & \textbf{+6.82\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Single Position (S6):} When positional flexibility is eliminated, early greedy mistakes are impossible to patch. OCG's foresight prevents these catastrophic positional deficits, maintaining high optimality.
