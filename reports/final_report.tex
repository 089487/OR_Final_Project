...
\]
\begingroup
\setlength{\jot}{-0.1pt} % 縮小行間距，可依需求調整數值
\textbf{Constraints:}
\begin{align}
...
& x_{ip} = 0 \quad \text{if } p \not\in E_i, && \forall i \in I, p \in P\label{eq:Ei} \\
...
\subsection{Direct Greedy (Baseline DG)}
The Direct Greedy algorithm (DG) serves as our baseline heuristic,  representing the standard ``fill the biggest need with the best available player'' mentality. At each pick, DG scans all open positions, identifies the position with highest scarcity ($\frac{\text{Remaining Need}}{\text{Available Players}}$), and selects the player with the highest projected points for that position.
...
\subsection{Factor C: Market Conditions and ADP Uncertainty ($\sigma_{adp}, \delta$)}
...
\begin{equation}
    \text{ADP}_i = \text{TrueRank}_i + \mathcal{N}(0, \sigma_{adp}^2)
\end{equation}

We evaluate two specific dimensions of market uncertainty:
\begin{itemize}
    \item \textbf{Market Volatility ($\sigma_{adp}$):} We vary $\sigma_{adp}$ from $0$, 30, to $60$. A value of $\sigma_{adp} = 0$ represents a perfectly efficient market where opponents draft strictly according to player value. Higher values of $\sigma_{adp}$ simulate ``chaotic'' markets where opponents may make unpredictable ``reaches'' for low-value players or allow elite assets to fall well past their expected rank. (fluctuated around $30/12 \approx 3$ and $60/12=5$ rounds)
    \item \textbf{Availability Tolerance ($\delta$):} This parameter represents the manager's risk threshold within the availability constraint ($S_k \le \text{ADP}_i + \delta$). We test values of $\delta \in \{0, 5, 10\}$. A negative $\delta$ represents a conservative, ``safety-first'' drafting strategy, while a positive $\delta$ allows the algorithm to be more aggressive, assuming players will remain available longer than the market average.
\end{itemize}

Testing under these conditions allows us to verify if the Opportunity Cost Greedy (OCG) algorithm can maintain its strategic edge even when the underlying market data is noisy or biased.
...
\begin{table}[H]
\centering
\caption{Factor D: Player Demand Ratio results (Fixed Roster Scale = 1). }
\label{tab:factor_4}
\begin{tabular}{lccc}
\toprule
Level & DG Gap (\%) & OCG Gap (\%) & Improvement (\%) \\
...
While the IP finds perfect rosters in smaller instances, what happens when the scouting department dumps a universe of hundreds of thousands of eligible players into the database? This presents the most critical argument for our front office pitch: \textbf{Scalability}. 
...
The Branch-and-Bound tree of the IP explores an exponentially growing space, and as we scale the roster size to 256 and the player pool to 184,320, the approximate variable count reaches nearly 50 million. Consequently, the IP runtime explodes and eventually hits our predefined 30-minute (1800 seconds) solver time limit for the largest target, triggering a \texttt{TIMEOUT\_ERROR}. In stark contrast, confirming our $O(n \log n)$ complexity analysis, \textbf{Opportunity Cost Greedy processes the massive 50-million variable equivalent environment in just 23.286 seconds}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{./runtime_by_variable_count.png}
...
The highlighted scenarios reveal where cost-of-delay reasoning matters most. OCG secures elite assets before the value cliff occurs (S3), prevents early positional mistakes that cannot be repaired later under single-position eligibility (S6), remains robust to uncertain and chaotic opponent behavior (S9), lastly, avoids ``positional blindness'' happened in DGs when facing tight markets (S11).
...
Future research could transition this deterministic framework toward \textbf{Stochastic Opponent Modeling}. Our current setting still simplifies many aspects of the real-world environment. By incorporating probability distributions of opponent behavior biases instead of fixed ADP thresholds, the model could calculate more precise expected opportunity costs. 
